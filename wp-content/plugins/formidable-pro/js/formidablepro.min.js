...
function(){jQuery(".wp-editor-area").each(function(){var isVisible="undefined"!==typeof tinyMCE.editors[this.id]&&!tinyMCE.editors[this.id].isHidden();if(isVisible){removeRichText(this.id);initRichText(this.id)}})})}function resetTinyMceOnPageTurn(){var removeIds=[];jQuery(".frm_form_field .wp-editor-area").each(function(){removeIds.push(this.id)});jQuery(document).one("frmPageChanged",function(){var removeIndex,removeId;for(removeIndex=0;removeIndex<removeIds.length;++removeIndex){removeId=removeIds[removeIndex];
removeRichText(removeId)}checkConditionalLogic()})}function toggleSection(e){var arrow;if(e.key!==undefined){if(e.key!==" ")return}else if(e.keyCode!==undefined&&e.keyCode!==32)return;e.preventDefault();jQuery(this).parent().children(".frm_toggle_container").slideToggle("fast");arrow=jQuery(this).children(".frm_arrow_icon");if(-1!==this.className.indexOf("active")){this.className=this.className.replace(" active","");arrow.attr("aria-expanded","false")}else{this.className+=" active";arrow.attr("aria-expanded",
"true")}}function loadDateFields(){jQuery(document).on("focusin",".frm_date",triggerDateField);loadUniqueTimeFields()}function triggerDateField(){if(this.className.indexOf("frm_custom_date")!==-1||typeof __frmDatepicker==="undefined")return;var dateFields=__frmDatepicker,id=this.id,idParts=id.split("-"),altID="";if(isRepeatingFieldByName(this.name))altID='input[id^="'+idParts[0]+'"]';else altID='input[id^="'+idParts.join("-")+'"]';jQuery.datepicker.setDefaults(jQuery.datepicker.regional[""]);var optKey=
0;for(var i=0;i<dateFields.length;i++)if(dateFields[i].triggerID==="#"+id||dateFields[i].triggerID==altID){optKey=i;break}if(dateFields[optKey].options.defaultDate!=="")dateFields[optKey].options.defaultDate=new Date(dateFields[optKey].options.defaultDate);jQuery(this).datepicker(jQuery.extend({},jQuery.datepicker.regional[dateFields[optKey].locale],dateFields[optKey].options))}function loadDropzones(repeatRow){if(typeof __frmDropzone==="undefined")return;var uploadFields=__frmDropzone;for(var i=
0;i<uploadFields.length;i++)loadDropzone(i,repeatRow)}function loadDropzone(i,repeatRow){var field,max,uploadedCount,form,uploadFields=__frmDropzone,uploadField=uploadFields[i],selector="#"+uploadField.htmlID+"_dropzone",fieldName=uploadField.fieldName;if(typeof repeatRow!=="undefined"&&selector.indexOf("-0_dropzone")!==-1){selector=selector.replace("-0_dropzone","-"+repeatRow+"_dropzone");fieldName=fieldName.replace("[0]","["+repeatRow+"]");delete uploadField.mockFiles}field=jQuery(selector);if(field.length<
...
0;m<files.length;m++)jQuery(files[m].previewElement).append(getHiddenUploadHTML(uploadFields[i],mediaIDs[m],fieldName));frmFrontForm.validateFormSubmit(files[0].previewElement.closest("form"))});this.on("complete",function(file){var fileName,node,img,thumbnail;processesRunning--;frmFrontForm.removeSubmitLoading(form,"enable",processesRunning);if(typeof file.mediaID==="undefined")return;if(uploadFields[i].uploadMultiple)jQuery(file.previewElement).append(getHiddenUploadHTML(uploadFields[i],file.mediaID,
fieldName));fileName=file.previewElement.querySelectorAll("[data-dz-name]");for(var _i=0,_len=fileName.length;_i<_len;_i++){node=fileName[_i];if(file.accessible)node.innerHTML='<a href="'+file.url+'" target="_blank" rel="noopener">'+file.name+"</a>";else node.innerHTML=file.name;if(file.ext){img=file.previewElement.querySelector(".dz-image img");if(null!==img){thumbnail=maybeGetExtensionThumbnail(file.ext);if(false!==thumbnail)img.setAttribute("src",thumbnail)}}}});this.on("addedfile",function(file){var ext,
...
checkFieldsOnPage();maybeShowMoreStepsButton()}function afterPageChanged(){checkFieldsOnPage();addTopAddRowBtnForRepeater();maybeDisableCheckboxesWithLimit();calcProductsTotal();maybeShowMoreStepsButton()}function generateGoogleTables(graphs,graphType){for(var num=0;num<graphs.length;num++)generateSingleGoogleTable(graphs[num],graphType)}function generateSingleGoogleTable(opts,type){google.load("visualization","1.0",{packages:[type],callback:function(){compileGoogleTable(opts)}})}function compileGoogleTable(opts){var data=
...
j,inputRanges;if(r.html){html=r.html;item=jQuery(html).addClass("frm-fade-in");thisBtn.parents(".frm_section_heading").append(item);inputRanges=item[0].querySelectorAll("input[type=range]");for(j=0;j<inputRanges.length;j++)handleSliderEvent.call(inputRanges[j]);if(r.is_repeat_limit_reached)hideAddButton(id);checked=["other"];reset="reset";repeatArgs={repeatingSection:id.toString(),repeatRow:i.toString()};jQuery(html).find("input, select, textarea").each(function(){if(this.name==="")return true;if(this.type==
"file")fieldID=this.name.replace("file","").split("-")[0];else fieldID=this.name.replace("item_meta[","").split("]")[2].replace("[","");if(jQuery.inArray(fieldID,checked)==-1){if(this.id===false||this.id==="")return;fieldObject=jQuery("#"+this.id);checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs);updateWatchingFieldById(fieldID,repeatArgs,"value changed");checkFieldsWithConditionalLogicDependentOnThis(fieldID,fieldObject);checkFieldsWatchingLookup(fieldID,fieldObject,"value changed");
doCalculation(fieldID,fieldObject);maybeDoCalcForSingleField(fieldObject.get(0));reset="persist"}});jQuery(html).find(".frm_html_container").each(function(){var fieldID=this.id.replace("frm_field_","").split("-")[0];checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs)});loadDropzones(repeatArgs.repeatRow);loadSliders();loadChosen();jQuery(html).find(".frm_html_container").each(function(){var fieldID=this.id.replace("frm_field_","").split("-")[0];checked.push(fieldID);hideOrShowFieldById(fieldID,
...
formEl);else{resetInputs(formEl);triggerCompletedEvent(formId)}}function triggerCompletedEvent(formId){triggerEvent(document,"frm_after_start_over",{formId:formId})}document.addEventListener("click",function(e){var target;for(target=e.target;target&&target!=this;target=target.parentNode)if(target.matches(".frm_start_over")){onClickStartOver.call(target,e);break}},false)}function maybeShowMoreStepsButton(){var i,j,listWrappers,listWrapper,rootlineSteps,wrappingElementsCount,startIndex,hiddenSteps,
showMoreButtonLi,showMoreButton,hiddenStepsWrapper,activeHiddenStepLink;listWrappers=document.querySelectorAll(".frm_page_bar.frm_rootline.frm_rootline_line");copyRootlines(listWrappers);for(i=0;i<listWrappers.length;i++){listWrapper=listWrappers[i];rootlineSteps=listWrapper.children;wrappingElementsCount=0;for(j=0;j<rootlineSteps.length;j++)if(rootlineSteps[j].offsetTop!==rootlineSteps[0].offsetTop)wrappingElementsCount++;if(!wrappingElementsCount)continue;startIndex=rootlineSteps.length-wrappingElementsCount>
1?rootlineSteps.length-wrappingElementsCount-2:0;hiddenSteps=[].slice.call(rootlineSteps,startIndex,rootlineSteps.length-1);showMoreButton=createShowMoreButton();showMoreButtonLi=createShowMoreButtonWrapper(showMoreButton);hiddenStepsWrapper=createHiddenStepsWrapper();hiddenSteps.forEach(function(hiddenStep){hiddenStepsWrapper.appendChild(hiddenStep);if(!hiddenStep.querySelector(".frm_rootline_title"))addRootlineTitleToStep(hiddenStep)});activeHiddenStepLink=hiddenStepsWrapper.querySelector("input:not(.frm_page_back):not(.frm_page_skip)");
if(activeHiddenStepLink){maybeAddLineThroughs(activeHiddenStepLink.parentElement.previousElementSibling,hiddenStepsWrapper);showMoreButton.className+=" active"}showMoreButtonLi.appendChild(hiddenStepsWrapper);listWrapper.insertBefore(showMoreButtonLi,listWrapper.children[listWrapper.children.length-1]);if(listWrapper.children[listWrapper.children.length-1].className.indexOf("frm_current_page")!==-1)addLineThrough(hiddenStepsWrapper)}}function copyRootlines(listWrappers){var i,listWrappers,listWrapper,
rootlinesBackup;rootlinesBackup={};for(i=0;i<listWrappers.length;i++){listWrapper=listWrappers[i];rootlinesBackup[listWrapper.closest("form").getAttribute("id")]=listWrapper.cloneNode(true)}listWrappersOriginal=rootlinesBackup}function maybeAddLineThroughs(previousPageLink,hiddenStepsWrapper){var i;if(previousPageLink){i=0;while(previousPageLink){i++;previousPageLink=previousPageLink.previousElementSibling}addLineThrough(hiddenStepsWrapper,i)}}function addLineThrough(hiddenStepsWrapper,uptoIndex){var rootlineTitles;
if(!uptoIndex)uptoIndex=hiddenStepsWrapper.children.length;rootlineTitles=[].slice.call(hiddenStepsWrapper.children,0,uptoIndex);rootlineTitles.forEach(function(el){el.querySelector(".frm_rootline_title").style.textDecoration="line-through"})}function createHiddenStepsWrapper(){var hiddenStepsWrapper=document.createElement("ul");hiddenStepsWrapper.className+=" frm_page_bar frm_rootline_hidden_steps";hiddenStepsWrapper.style.setProperty("display","none","important");return hiddenStepsWrapper}function addRootlineTitleToStep(hiddenStep){var rootlineTitle=
document.createElement("span");rootlineTitle.className="frm_rootline_title";rootlineTitle.textContent=hiddenStep.querySelector("input").getAttribute("aria-label");hiddenStep.appendChild(rootlineTitle)}function showMoreSteps(e){var hiddenStepsWrapper=e.target.parentElement.querySelector("ul");hiddenStepsWrapper.style.setProperty("display",hiddenStepsWrapper.style.display==="none"?"flex":"none","important")}function createShowMoreButton(){var showMoreButton=document.createElement("input");showMoreButton.type=
"button";showMoreButton.className+=" frm_rootline_show_more_btn";showMoreButton.addEventListener("click",showMoreSteps);showMoreButton.value="...";return showMoreButton}function createShowMoreButtonWrapper(showMoreButton){var showMoreButtonLi=document.createElement("li");showMoreButtonLi.className+=" frm_rootline_single frm_rootline_show_hidden_steps_btn";showMoreButtonLi.appendChild(showMoreButton);return showMoreButtonLi}function maybeAddPolyfills(){if(!Element.prototype.matches)Element.prototype.matches=
...
window.performance&&window.performance.getEntriesByType("navigation")[0].type==="back_forward")triggerCalc()}function showMoreStepsButtonEvents(){function debounce(func,wait){var timeout;return function(){clearTimeout(timeout);timeout=setTimeout(function(){func.apply(this,arguments)},wait)}}var debounceShowMoreButton=debounce(maybeShowMoreStepsButton,500);window.addEventListener("resize",function(){var i,listWrappers,listWrapper,form;listWrappers=document.querySelectorAll(".frm_page_bar.frm_rootline.frm_rootline_line");
for(i=0;i<listWrappers.length;i++){listWrapper=listWrappers[i];form=listWrapper.closest("form");form.querySelector(".frm_rootline_group").replaceChild(listWrappersOriginal[form.getAttribute("id")],listWrapper)}debounceShowMoreButton()});document.addEventListener("click",function(e){var i,hiddenRootlineWrappers,closestHiddenRootlinesWrapper,hiddenRootlineWrapper;if(typeof e.target.className!=="string")return;if(e.target.className.indexOf("frm_rootline_single")!==-1||e.target.className.indexOf("frm_rootline_title")!==
-1)setNextPage.call(e.target);closestHiddenRootlinesWrapper=e.target.closest(".frm_rootline_hidden_steps");if(closestHiddenRootlinesWrapper||e.target.className.indexOf("frm_rootline_show_more_btn")!==-1)return;hiddenRootlineWrappers=document.getElementsByClassName("frm_rootline_hidden_steps");for(i=0;i<hiddenRootlineWrappers.length;i++){hiddenRootlineWrapper=hiddenRootlineWrappers[i];hiddenRootlineWrapper.style.display="none"}})}return{init:function(){maybeAddPolyfills();addEventListener("pageshow",
maybeTriggerCalc);jQuery(document).on("frmFormComplete",afterFormSubmitted);jQuery(document).on("frmPageChanged",afterPageChanged);jQuery(document).on("frmAfterAddRow frmAfterRemoveRow",calcProductsTotal);jQuery(document).on("click",".frm_trigger",toggleSection);jQuery(document).on("keydown",".frm_trigger",toggleSection);var $blankField=jQuery(".frm_blank_field");if($blankField.length)$blankField.closest(".frm_toggle_container").prev(".frm_trigger").trigger("click");jQuery(document).on("click",".frm_remove_link",
...
addRteRequiredMessages();setAutoHeightForTextArea();updateContentLength();calcProductsTotal();startOverButton();maybeShowMoreStepsButton();showMoreStepsButtonEvents();validateFieldValue()},savingDraft:function(object){return savingDraftEntry(object)},goingToPreviousPage:function(object){return goingToPrevPage(object)},hideOrShowFields:function(ids,event){if("pageLoad"===event)clearHideFields();var len=ids.length,repeatArgs={repeatingSection:"",repeatRow:""};for(var i=0,l=len;i<l;i++){hideOrShowFieldById(ids[i],
repeatArgs);if(i==l-1)showForm()}},hidePreviouslyHiddenFields:function(){var hiddenFields=getAllHiddenFields(),len=hiddenFields.length;for(var i=0,l=len;i<l;i++){var container=document.getElementById(hiddenFields[i]);if(container==null){container=document.querySelector("#"+hiddenFields[i]);if(container!=null&&hiddenFields[i].indexOf("frm_final_submit")>-1){hidePreviouslyHiddenSubmitButton(hiddenFields[i]);continue}}if(container!==null){container.style.display="none";setHiddenProductContainer(container)}}},
submitAllowed:function(object){var formElementId=object.getAttribute("id");if(!isSubmitButtonOnPage(formElementId+" .frm_final_submit")||goingToPrevPage(object)||savingDraftEntry(object))return true;var formKey=getFormKeyFromFormElementID(formElementId);return!isOnPageSubmitButtonHidden(formKey)},checkDependentDynamicFields:function(ids){var len=ids.length,repeatArgs={repeatingSection:"",repeatRow:""};for(var i=0,l=len;i<l;i++)hideOrShowFieldById(ids[i],repeatArgs)},checkDependentLookupFields:function(ids){var fieldId,
repeatArgs={repeatingSection:"",repeatRow:""};for(var i=0,l=ids.length;i<l;i++){fieldId=ids[i];updateWatchingFieldById(fieldId,repeatArgs,"value changed")}},loadGoogle:function(){var graphs,packages,i;if(typeof google==="undefined"||!google||!google.load){setTimeout(frmProForm.loadGoogle,30);return}graphs=__FRMTABLES;packages=Object.keys(graphs);for(i=0;i<packages.length;i++)if(packages[i]==="graphs")generateGoogleGraphs(graphs[packages[i]]);else generateGoogleTables(graphs[packages[i]],packages[i])},
removeUsedTimes:function(obj,timeField){var $form,form,e,data,success,extraParams;$form=jQuery(obj).parents("form").first();form=$form.get(0);e=$form.find('input[name="id"]');data={action:"frm_fields_ajax_time_options",time_field:timeField,date_field:obj.id,entry_id:e?e.val():"",date:jQuery(obj).val(),nonce:frm_js.nonce};success=function(opts){var $timeField=jQuery(document.getElementById(timeField));$timeField.find("option").prop("disabled",false);if(opts.length>0)for(var i=0,l=opts.length;i<l;i++)$timeField.get(0).querySelectorAll('option[value="'+
opts[i]+'"]').forEach(function(option){option.disabled=true;if(option.selected)option.selected=false})};extraParams={dataType:"json"};postToAjaxUrl(form,data,success,false,extraParams)},changeRte:function(editor){editor.on("change",function(){var content=editor.getBody().innerHTML;jQuery("#"+editor.id).val(content).trigger("change")})},addFormidableClassToDatepicker:function(_,options){if(options.dpDiv){options.dpDiv.addClass("frm-datepicker");options.dpDiv.get(0).setAttribute("is-formidable-datepicker",
