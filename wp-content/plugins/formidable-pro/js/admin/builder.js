( function() {
	...
	function appendModalTriggersToRtePlaceholderSettings() {
		...
		rtePlaceholderDefaults.forEach( function( defaultValueWrapper ) {
	...
	function copyChangemeFromWrapperToInput( defaultValueWrapper ) {
		document.getElementById( defaultValueWrapper.getAttribute( 'data-html-id' ) ).setAttribute( 'data-changeme', defaultValueWrapper.getAttribute( 'data-changeme' ) );
	...
	document.addEventListener( 'frm_added_field', event => {
		if ( 'rte' !== event.frmType ) {
			return;
		}
		const defaultValueWrapper = document.getElementById( 'default-value-for-' + event.frmField.getAttribute( 'data-fid' ) );
		addSmartValuesTriggerToDefaultValueWrapper( defaultValueWrapper );
		copyChangemeFromWrapperToInput( defaultValueWrapper );
	});
