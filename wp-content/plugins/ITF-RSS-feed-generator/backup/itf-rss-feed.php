...
$query = "UPDATE  rss_feed_url SET merge_url = '".$jemi."' WHERE id IN (".$jkons.");";
    ...
    $querys = "INSERT INTO feed_url_cron (merge_url) VALUES ('".$jemi."')";
...
<div class="add_btn"><a href="<?php echo get_site_url(); ?>/wp-admin/admin.php?page=rss_feed_generator"><input type="button" name="Add_feed" value="Generate New Merge"></a></div>
<table id="table_id" class="display table_trade">
    ...
    <tr>
        ...
        <td>
            <a target="_blank" href="<?php echo get_site_url().'/wp-admin/admin.php?page=update_channel&merge_name='.$key->merge_name; ?>">Update Channel Info</a>
        </td>
        <td>
            <a target="_blank" href="<?php echo get_site_url().'/wp-admin/admin.php?page=feed_url_list&merge_name='.$key->merge_name; ?>">Add / Remove Feeds</a>
...
jQuery('.delete-feed').click(function() {
  console.log('<?php echo get_site_url();?>/wp-admin/admin.php?page=rss_feed_list&merge_name='+$(this).attr('data-id'));
         window.location.replace('<?php echo get_site_url();?>/wp-admin/admin.php?page=rss_feed_list&merge_name='+$(this).attr('data-id'));
    });
...
<form style="display: none;" method="get" action="<?php echo get_site_url().'/wp-admin/admin.php?page=feed_url_list'; ?>" name="add_feed" class="add_feed">
...
<script type="text/javascript">
    ...
    jQuery('.delete').click(function() {
        //console.log('<?php echo get_site_url().'/my-feed-list?merge_name='.$key->merge_name.'&'; ?>delete_id='+$(this).attr('data-id'));
         window.location.replace('<?php echo get_site_url().'/wp-admin/admin.php?page=feed_url_list&merge_name='.$key->merge_name.'&'; ?>delete_id='+$(this).attr('data-id'));
    });
...
<form name="chanel_update" method="post" action="<?php echo get_site_url().'/wp-admin/admin.php?page=update_channel&merge_name='.urlencode($_REQUEST['merge_name']); ?>">
...
function feed_settings(){
 ...
 //  to show the subscription levels in settings page
      global $wpdb;

        $get_sub = $wpdb->get_results("SELECT * FROM wp_pmpro_membership_levels;");        
print_r($get_sub[2]->name); die;
    for ($i=0; $i < count($get_sub); $i++) { 
        feed_generator($get_merge_url[$i]->merge_url);
    }
}
