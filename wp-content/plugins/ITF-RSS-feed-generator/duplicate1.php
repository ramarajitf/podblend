...
var adminURL = siteURL + '/wp-admin/admin.php?page=rss_feed_list&merge_name=' + feedID;
...
jQuery.ajax({
  ...
  success: function(response) {
    ...
    // Perform any necessary actions after the feed has been deleted
      },
...
//jQuery('.delete-feed').click(function() {
  //console.log('<?php echo esc_url(get_site_url());?>/rss-feeds/?merge_name='+$(this).attr('data-id'));
    //     window.location.replace('<?php echo esc_url(get_site_url());?>/rss-feeds/?merge_name='+$(this).attr('data-id'));
   // });
...
data: {"action": "delete_merge_feed", "id" : id, "nonce": my_ajax_object.nonce },
...
<?php  
add_action('wp_ajax_delete_merge_feed', 'delete_merge_row');
add_action('wp_ajax_nopriv_delete_merge_feed', 'delete_merge_row');

function delete_merge_row(){

global $wpdb;
$id = $POST['id'];
$wpdb->delete('feed_url_cron', array( 'merge_name' => urldecode($POST['id'])));
...
<?php   if(is_admin()) { ?>
   ...
   <?php }  else{ ?>
    ...
    jQuery('.delete').click(function() {
         window.location.replace('<?php echo get_site_url().'/url-list/?merge_name='.$key->merge_name.'&'; ?>delete_id='+$(this).attr('data-id'));
    });
<?php   }  ?>   
